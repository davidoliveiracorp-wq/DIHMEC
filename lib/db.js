...
// A integracao Neon na Vercel exige um "Custom Prefix" quando voce
// conecta o banco a um projeto que ja tem outras integracoes (no caso
// do dihmec o usuario teve que usar prefixo "STORAGE"). Alem disso, a
// integracao Neon nova nomeia as vars como DATABASE_URL (padrao Neon),
// nao como URL/POSTGRES_URL (padrao antigo do Vercel Postgres).
//
// O cliente @vercel/postgres so sabe ler POSTGRES_URL. Aqui aliasamos
// todas as combinacoes conhecidas para os nomes padrao ANTES da
// primeira chamada `sql` (que cria o pool lazy). Como `sql` so le o
// env na primeira chamada, fazer isso no top-level do modulo basta.
const ENV_PREFIXES = ['', 'STORAGE_', 'DIHMEC_DB_', 'NEON_'];

// Mapeia <source name suffix> -> <target POSTGRES_* suffix>
// Ex.: STORAGE_DATABASE_URL -> POSTGRES_URL,
//      STORAGE_DATABASE_URL_UNPOOLED -> POSTGRES_URL_NON_POOLING.
const ENV_MAP = [
  ['DATABASE_URL', 'URL'],
  ['DATABASE_URL_UNPOOLED', 'URL_NON_POOLING'],
  ['URL', 'URL'],
  ['URL_NON_POOLING', 'URL_NON_POOLING'],
...
for (const prefix of ENV_PREFIXES) {
  for (const [sourceSuffix, targetSuffix] of ENV_MAP) {
    const target = 'POSTGRES_' + targetSuffix;
    const source = prefix + sourceSuffix;
    if (source === target) continue; // ja eh o nome canonico
...
export async function ensureSuperAdmins() {
  ...
  for (const email of SUPER_ADMIN_EMAILS) {
    ...
    // Reseta a senha APENAS quando o usuario ainda tem um seedTag (ou
    // seja, esta com uma senha-padrao de versao anterior) E esse tag eh
    // diferente do atual. Se o seedTag ja foi removido (a senha foi
    // alterada manualmente via /api/change-password), preservamos.
    if (user.seedTag && user.seedTag !== seedTag) {
