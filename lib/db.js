...
// A integracao Neon na Vercel exige um "Custom Prefix" obrigatorio
// quando voce conecta o banco a um projeto que ja tem outras integracoes
// (no nosso caso o usuario teve que usar prefixo "STORAGE"). Isso cria
// as vars como STORAGE_URL, STORAGE_PRISMA_URL, etc.
//
// O cliente @vercel/postgres so sabe ler POSTGRES_URL. Aliasamos os
// nomes prefixados para os nomes padrao ANTES da primeira chamada
// `sql` (que cria o pool e le o env). Como `sql` eh lazy, fazer isso
// no top-level do modulo eh suficiente.
const ENV_ALIAS_PREFIXES = ['STORAGE_', 'DIHMEC_DB_', 'NEON_'];
const ENV_SUFFIXES = [
  'URL',
  'PRISMA_URL',
  'URL_NON_POOLING',
